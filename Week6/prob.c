...
void sortedInsert( Node** head,  Node* newNode)
{
    ...
    while (current->next != NULL && current->next->count >= newNode->count) {
        if (current->next->count == newNode->count) {//카운트 수가 같으면 word에 대해서 sorting.
            if (strcmp(current->next->word, newNode->word)>0) {
                strcpy(temp, current->next->word);
                strcpy(current->next->word, newNode->word);
